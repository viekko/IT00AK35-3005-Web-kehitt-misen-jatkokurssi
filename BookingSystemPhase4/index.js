...
pool
  ...
  .then((client) => {
    client.release();
    console.log(`[${timestamp()}] ✅ DB connected`);
  })
  .catch((err) => {
    console.error(`[${timestamp()}] ❌ DB connection failed`, err);
  });
...
const resourceValidators = [
  ...
  body("resourceName")
    ...
    .trim()
    .isLength({ min: 2, max: 60 }).withMessage("resourceName must be 2–60 characters")
    .custom((v) => !/[<>]/.test(v)).withMessage("resourceName cannot contain HTML tags"),
  ...
  body("resourceDescription")
    ...
    .trim()
    .isLength({ min: 10, max: 500 }).withMessage("resourceDescription must be 10–500 characters")
    .custom((v) => !/[<>]/.test(v)).withMessage("resourceDescription cannot contain HTML tags"),
  ...
  body("resourcePrice")
    ...
    .isFloat({ min: 0.01 }).withMessage("resourcePrice must be > 0")
    .toFloat(),
...
app.post("/api/resources", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    `;
    const params = [
      resourceName,          // ✅ ei hashata
      resourceDescription,   // ✅ alkuperäinen (tagiton, validoitu)
      resourceAvailable,     // ✅ ei yliajoa
      resourcePrice,         // ✅ ei *2
      resourcePriceUnit,     // ✅ hyväksytyistä arvoista
    ];
...
// Health-check (helpottaa debuggausta ja composea)
app.get("/health", (_req, res) => res.json({ ok: true, time: timestamp() }));
