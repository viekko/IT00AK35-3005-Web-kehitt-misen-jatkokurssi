 ...
 * @param {string} params.action - e.g. "Login"
 ...
 */
export async function logEvent({ actorUserId = null, action, message, entityType = null, entityId = null }) {
  if (!message || typeof message !== "string") {
    throw new Error("logEvent: message must be a non-empty string");
  }

  const sql = `
    INSERT INTO system_log (actor_user_id, action, message, entity_type, entity_id)
    VALUES ($1, $2, $3, $4, $5)
  `;

  try {
    await pool.query(sql, [actorUserId, action, message, entityType, entityId]);
  } catch (err) {
