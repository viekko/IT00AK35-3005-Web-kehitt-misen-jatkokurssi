...
router.post("/", resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    await logEvent({ // TÄHÄN LISÄTTY KORJAUS!!!
      actorUserId: null,
      message: `RESOURCE_CREATE_FAILED_VALIDATION: invalid input for name="${req.body.resourceName || ""}" (400 Bad Request)`,
      entityType: "resource",
      entityId: null,
    });
  ...
  try {
    ...
    // Add log event
    // TÄHÄN LISÄTTY KORJAUS!!!
    const resourceId = rows[0].id;
    await logEvent({
      actorUserId,
      message: `RESOURCE_CREATED: id=${resourceId}, name="${resourceName}", price=${resourcePrice}, unit="${resourcePriceUnit}"`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE_CREATE_BLOCKED_DUPLICATE: name="${resourceName}" (409 Conflict)`, // TÄHÄN LISÄTTY KORJAUS!!!
        entityType: "resource",
        entityId: null,
      });
